...
**编程记录 (CodeTracker)** - 少儿编程学习追踪系统，面向教师的单用户工具。用于记录少儿编程弟子的学习情况，生成月度学习报告PDF发给家长。
...
│   │   ├── rich-editor.tsx  # TipTap 富文本编辑器（支持加粗/下划线/标题/颜色/背景/图片粘贴/代码插入/绘制文本框/箭头/画板）
│   │   ├── table-of-contents.tsx # 目录组件（自动提取 H1/H2/H3 标题，点击跳转）
│   │   └── drawing-canvas.tsx # 画板弹窗组件（画笔/箭头/颜色/撤销/保存为图片）
│   ├── hooks/              # 自定义 Hooks
│   ├── lib/
...
- **Course**: 课程（C++信奥/Python/图形化），含授课体系(curriculum树结构)、自定义知识点和题目
- **CurriculumNode**: 授课体系节点（章/节/知识点），支持内容描述、代码示例、关联知识点和题目
...
| `/courses/[id]` | 课程详情 - 授课体系(树编辑器) / 知识点(含子题目) |
